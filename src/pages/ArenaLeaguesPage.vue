<template>
  <q-page class="q-pa-lg">
    <PageHeader :title="$t('leagues.title')" :subtitle="$t('leagues.subtitle')">
      <q-btn
        color="primary"
        icon="add"
        :label="$t('leagues.new_league')"
        unelevated
        no-caps
        class="text-bold"
      ...
      <template v-else-if="leagueStore.leagues.length > 0">
        ...
        >
          <LeagueCard
            ...
            @ranking="
              $router.push({ name: 'league-ranking', params: { id: arenaId, leagueId: league.id } })
            "
            @active-stage="
              league.active_stage &&
              $router.push({
                name: 'stage-detail',
                params: { id: arenaId, leagueId: league.id, stageId: league.active_stage.id },
              })
            "
          />
        </div>
    ...
    <BtDialog
      v-model="showDialog"
      :title="form.id ? $t('leagues.dialog_edit') : $t('leagues.dialog_new')"
      closable
    >
      <div class="q-gutter-y-lg">
        ...
        <div class="row q-col-gutter-lg">
          <div class="col-12 col-sm-4">
            <div class="q-gutter-y-xs">
              ...
              <BtInput
                ...
                type="number"
                :placeholder="$t('leagues.field_stages_placeholder')"
              />
            </div>
          </div>
          <div class="col-12 col-sm-4">
          ...
          <div class="col-12 col-sm-4">
            <div class="q-gutter-y-xs">
              ...
              <BtSelect v-model="form.gender" :options="generoOptions" emit-value map-options />
            </div>
          </div>
        </div>
        ...
        <div class="q-gutter-y-xs">
          ...
          <BtTextarea
            ...
            :placeholder="$t('leagues.field_description_placeholder')"
            :rows="3"
          />
        </div>
        ...
        <div class="q-gutter-y-xs">
          ...
          <BtTextarea
            ...
            :placeholder="$t('leagues.field_award_placeholder')"
            :rows="3"
          />
        </div>
      </div>
...
function editLeague(league) {
  ...
  form.gender = league.gender ?? 'mixed';
