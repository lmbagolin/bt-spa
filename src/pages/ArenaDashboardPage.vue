    ...
    <div class="row q-col-gutter-lg">
        ...
        <div class="row items-center justify-between no-wrap">
          <div>
            ...
            <p class="text-sm text-surface-500 q-mt-xs q-mb-none font-medium">
              Visão geral da Arena <span class="text-primary">{{ arenaStore.currentArena?.name }}</span>
            </p>
          </div>
          <div>
            <q-btn
      ...
      <div class="col-12 col-sm-6 col-md-3">
        <q-card class="stat-card bg-gradient-blue text-white border-none relative-position">
          <q-card-section class="q-pa-lg">
            <div class="text-xs text-uppercase text-weight-bold opacity-80 q-mb-xs tracking-widest">TOTAL TORNEIOS</div>
            <div class="text-h4 text-weight-bolder">{{ arenaStore.dashboard.total_tournaments }}</div>
      ...
      <div class="col-12 col-sm-6 col-md-3">
        <q-card class="stat-card bg-gradient-purple text-white border-none relative-position">
          <q-card-section class="q-pa-lg">
            <div class="text-xs text-uppercase text-weight-bold opacity-80 q-mb-xs tracking-widest">JOGADORES</div>
      ...
      <div class="col-12 col-sm-6 col-md-3">
        <q-card class="stat-card bg-gradient-green text-white border-none relative-position">
          <q-card-section class="q-pa-lg">
            <div class="text-xs text-uppercase text-weight-bold opacity-80 q-mb-xs tracking-widest">TORNEIOS ATIVOS</div>
            <div class="text-h4 text-weight-bolder">{{ arenaStore.dashboard.active_tournaments || 1 }}</div>
      ...
      <div class="col-12 col-sm-6 col-md-3">
        <q-card class="stat-card bg-gradient-amber text-white border-none relative-position">
          <q-card-section class="q-pa-lg">
            <div class="text-xs text-uppercase text-weight-bold opacity-80 q-mb-xs tracking-widest">MENSAGENS</div>
      ...
      <div class="col-12 col-md-8">
        <q-card class="sakai-card-plain full-height overflow-hidden shadow-card">
          ...
          <q-card-section class="q-pa-none">
            <q-list v-if="arenaStore.dashboard.recent_activities && arenaStore.dashboard.recent_activities.length > 0">
              <q-item v-for="(activity, index) in arenaStore.dashboard.recent_activities" :key="index" class="q-px-xl q-py-md item-border">
                <q-item-section avatar>
                  <q-avatar :color="getActivityIcon(activity).color" :text-color="getActivityIcon(activity).textColor" :icon="getActivityIcon(activity).icon" size="32px" />
                </q-item-section>
                <q-item-section>
                  <q-item-label class="text-weight-bold text-surface-800">{{ activity.message }}</q-item-label>
                  <q-item-label caption class="text-surface-400">há {{ activity.time }}</q-item-label>
                </q-item-section>
              </q-item>
            </q-list>
        ...
        <div class="column q-gutter-y-lg">
          ...
          <q-card class="sakai-card-plain shadow-card">
            ...
            <q-card-section class="q-pa-lg q-gutter-y-sm">
              <q-btn outline color="primary" icon="emoji_events" label="Novo Torneio" class="full-width text-left q-py-md btn-quick" align="left" no-caps @click="openTournamentForm" />
              <q-btn outline color="primary" icon="person_add" label="Cadastrar Jogador" class="full-width text-left q-py-md btn-quick" align="left" no-caps />
              <q-btn outline color="primary" icon="tune" label="Ajustar Categorias" class="full-width text-left q-py-md btn-quick" align="left" no-caps />
            </q-card-section>
          </q-card>
          ...
          <q-card class="sakai-card-plain shadow-card">
            <q-card-section class="q-px-xl q-py-lg">
              <div class="text-xs text-bold text-surface-300 uppercase tracking-widest">ÚLTIMOS TORNEIOS</div>
            </q-card-section>
            <q-separator color="surface-100" />
            <q-card-section class="q-pa-none">
              <q-list separator>
                ...
                ]" :key="t.name" class="q-py-md">
                  <q-item-section>
                    <q-item-label class="text-bold text-surface-800">{{ t.name }}</q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <q-badge :color="t.color + '-1'" :text-color="t.color + '-10'" class="text-bold q-px-sm" :label="t.status" />
                  </q-item-section>
                </q-item>
              </q-list>
            </q-card-section>
        ...
        <q-form @submit="onSubmitTournament">
          <q-card-section class="q-pa-xl q-gutter-y-xl">
            <div class="q-gutter-y-xs">
              <label class="text-xs font-bold text-surface-500 block uppercase tracking-widest q-mb-sm">Nome do Torneio *</label>
              <q-input
            ...
            <div class="row q-col-gutter-lg">
              <div class="col-12 col-sm-6">
                <label class="text-xs font-bold text-surface-500 q-mb-sm block uppercase tracking-widest">Tipo *</label>
                <q-select
              ...
              <div class="col-12 col-sm-6">
                <label class="text-xs font-bold text-surface-500 q-mb-sm block uppercase tracking-widest">Status Inicial *</label>
                <q-select
            ...
            <div class="info-banner q-pa-lg flex no-wrap items-center">
              ...
              <div class="text-sm text-surface-700 leading-normal">
                Comece como <strong class="text-primary font-bold">Rascunho</strong> para configurar as chaves e premiação antes de abrir as inscrições públicas.
              </div>
            </div>
          </q-card-section>

          <q-card-actions align="right" class="q-pa-xl bg-surface-50 border-surface-100">
            <q-btn flat label="Descartar" v-close-popup color="surface-500" no-caps class="text-bold" />
            <q-btn
