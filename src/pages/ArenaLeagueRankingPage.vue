<template>
  <q-page class="q-pa-lg">
    ...
    <template v-else>
      <PageHeader
        :title="`${$t('league_ranking.title_prefix')} — ${leagueStore.currentLeague?.nome ?? ''}`"
        ...
        :breadcrumbs="[
          ...
          { label: $t('league_ranking.breadcrumb') },
        ]"
        :back="() => $router.back()"
      />
      <!-- Empty state -->
      <div v-if="!stages.length" class="text-center q-pa-xl">
        ...
        <div class="text-lg text-bold text-surface-400 q-mb-sm">{{ $t('league_ranking.empty_title') }}</div>
        <div class="text-sm text-surface-300">
          {{ $t('league_ranking.empty_subtitle') }}
        </div>
      </div>
      ...
      <template v-else>
        ...
        <div v-if="podiumTop3.length" class="podium-row q-mb-xl">
          <template
            v-for="entry in [podiumTop3[1], podiumTop3[0], podiumTop3[2]]"
            :key="entry?.player?.id"
          >
            <div
              v-if="entry"
              class="podium-card"
              :class="{
                'podium-1': entry.position === 1,
                'podium-2': entry.position === 2,
                'podium-3': entry.position === 3,
              }"
            >
              <q-icon
              ...
              <div
                class="podium-medal"
                :class="{
                  'medal-gold':   entry.position === 1,
                  'medal-silver': entry.position === 2,
                  'medal-bronze': entry.position === 3,
                }"
              >
                {{ entry.position }}°
              </div>
              <div
                class="player-mini-av q-mb-sm"
                :class="{ 'player-mini-av--lg': entry.position === 1 }"
                :style="{ background: entry.player?.color || '#64748b' }"
              >
                {{ getInitial(entry.player?.name) }}
              </div>
              <div
                class="podium-name"
                :class="{ 'podium-name--bold': entry.position === 1 }"
              >
                {{ entry.player?.name || '—' }}
              </div>
              <div
                class="podium-pts"
                :class="{ 'podium-pts--gold': entry.position === 1 }"
              >
        ...
        <div class="text-xs text-bold text-surface-400 uppercase tracking-widest q-mb-sm">
          {{ $t('league_ranking.table_title') }}
        </div>
        <q-card class="border-surface-100 shadow-card overflow-hidden">
          <!-- Header -->
          <div class="ranking-header" :style="gridStyle">
            <span class="rh rh-center">{{ $t('league_ranking.col_pos') }}</span>
            <span class="rh rh-left">{{ $t('league_ranking.col_player') }}</span>
            <span class="rh rh-center">{{ $t('league_ranking.col_total') }}</span>
            <span class="rh rh-center">{{ $t('league_ranking.col_wins') }}</span>
            <span class="rh rh-center">{{ $t('league_ranking.col_sg') }}</span>
            <span class="rh rh-center">{{ $t('league_ranking.col_gp') }}</span>
            <span class="rh rh-center">{{ $t('league_ranking.col_gc') }}</span>
            <span
              v-for="s in stages"
              :key="s.id"
              class="rh rh-center"
              :title="`${tipoLabel(s.tipo)} — ${formatDate(s.data_etapa)}`"
            >
              {{ formatStageLabel(s) }}
            </span>
          </div>
          ...
          >
            ...
            <span
              ...
              :class="
                entry.total_gp - entry.total_gc > 0
                  ? 'rr-pos-val'
                  : entry.total_gp - entry.total_gc < 0
                    ? 'rr-neg-val'
                    : ''
...
// Grid: pos | player | total | V | SG | GP | GC | one col per stage
...
const podiumTop3 = computed(() =>
  rankings.value.filter((e) => e.position <= 3).slice(0, 3),
);
...
onMounted(async () => {
  ...
  try {
    ...
    await leagueStore.fetchLeagueRanking(arenaId, leagueId);
  } finally {
    loading.value = false;
  }
});
...
function formatStageLabel(s) {
  ...
  const d = new Date(s.data_etapa.replace(' ', 'T'));
...
.rr-cell--total  { font-size: 0.82rem; font-weight: 800; color: #1b2e6e; }
